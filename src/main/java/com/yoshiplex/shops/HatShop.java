...
public class HatShop implements Listener{
	private final int width = 9;
	private final int height = 5;
	private final String title = ChatColor.GREEN + "--------" +  "  " + ChatColor.RED + "HAT SHOP" + "  " + ChatColor.GREEN + "--------";
	private Inventory inv = Bukkit.createInventory(null, width * height, title);
	
	public HatShop (Plugin p){
		Bukkit.getPluginManager().registerEvents(this, p);
		Inventory creation = ObjectChanger.addTilesFor45(inv);
		
		
		for(Hats hat : Hats.getAll()){
			creation.addItem(hat.getItem());
		}
		inv = creation;
	...
	public void onClick(InventoryClickEvent e){
		...
		Hats request = null;
		for(Hats hat : Hats.getAll()){
			if(hat.getItem().getType() == m){
				request = hat;
				break;
			}
		}
		if(request != null){
			if(Main.getConfigVar().getStringList("players." + p.getUniqueId() + ".hats").contains(request.getName())){
			...
			if(oneups >= price){
				List<String> hats = Main.getConfigVar().getStringList("players." + p.getUniqueId() + ".hats");
				hats.add(request.getName());
				p.sendMessage(ChatColor.GREEN + "You have just purchased the " + ChatColor.RED + request.getName() + ChatColor.GREEN + " hat! Do /hat " + request.getName() + " to wear it!");
				Main.getConfigVar().set("players." + p.getUniqueId() + ".hats", hats);
				oneups -= price;
	...
	public void onCommand(PlayerCommandPreprocessEvent e) {
		...
		if(e.getMessage().toLowerCase().startsWith("/hatshop")) {
			e.setCancelled(true);
			p.openInventory(inv);
		}
	}
